...
for file in os.listdir(foci_folder):
    if 'Probabi' in file:
        ...
        foci_orig = h.File(os.path.join(foci_folder, basename + '.h5'))['data'][:,:]

        # load mitochondria 
        mito_mask = h.File(os.path.join(mito_segmentation_folder,basename + extension + '_Simple_Segmentation.h5'))['exported_data'][:,:,0]
...
res = pd.DataFrame(res, columns=['image','dna_foci','mito_length','mito_area'])
